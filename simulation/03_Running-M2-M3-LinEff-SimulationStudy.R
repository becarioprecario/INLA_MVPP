###########################################################################
### Simulation study IIb: Running models with lineal covariate effect. ####
###########################################################################

# In this script we run model 2 and  model 3 modelling the exposure effect
# using a linner effect. As the other models, these ones will be computed 
...
for(m in 1:6){
  ...
  # Estimation of the intensity for both cases and controls 
  ...
  guardar <- paste0("M2_M3_Leff_cases", n.cases[m] , "phi3.RData" )
  
  save.image(guardar)
  
  
  rm(cases, cases_ppp, cls.sim, controls_ppp, guardar, mesh, ti, n.cases, 
     n.controls, p, pp.res.M2.Leff, pp.res.M3.Leff, pts_ppp, pts.dens, 
