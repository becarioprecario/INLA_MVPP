############################################################################
##### Simulation study III: Running models with Rw1 covariate effect. ######
############################################################################

# In this script we run model 2 and  model 3 modelling the exposure effect
# using a order 1 random walk effect. As the other models, these ones will
# be computed using the different groups of cases in order to study the 
# ability of each model to highlight the effect depending on the available 
# sample size. 
...
for(m in 1:6){
  ...
  form <- as.formula( 
    paste0("y ~ ", 
      ...
      " + ",
      paste(paste0("f(inla.group(", paste0("var", p, ".", 2), ", n=10), 
  ...
  form <- as.formula( 
    paste0("y ~ ", 
      ...
      f(cont.copy2, copy = "spatial.field1", fixed = TRUE)' , 
      " + ",
      paste(paste0("f(inla.group(", paste0("var", p, ".", 2), ", n=50),
        model='rw1', scale.model=TRUE)"), collapse = " + " )
    )
  )
  ...
  guardar <- paste0("M2_M3_Rw1_cases", n.cases[m] , "phi3.RData" )
  
  save.image(guardar)
  
  
  rm(cases, cases_ppp, cls.sim, controls_ppp, guardar, mesh, ti, n.cases, 
     n.controls, p, pp.res.M2.Rw1, pp.res.M3.Rw1, pts_ppp, pts.dens, 
